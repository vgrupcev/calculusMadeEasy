...
how \emph{we can so split it. In order to find this out
...
\emph{Case I.} If we perform many additions of two or
more fractions the denominators of which contain only
terms in $x$, and no terms in $x^2$, $x^3$, or any other powers
of $x$, we always \emph{find that the denominator of the final
...
sign between the partial fractions to be plus,\emph{ since, if
...
$4 = (A \times 0)+(B \times 2)$, so that $B=2$; and if we make
$x=-1$, we get $-2 = (A \times -2) + (B \times 0)$, so that $A=1$.
...
\[
-8 = (A \times 0) + B(2 \times 4) + (C \times 0);\quad \text{that is, } B = -1.
\]

If $x= -1$, we get:
\[
-12 = A(-2 \times 2) + (B \times 0) + (C \times 0);\quad \text{whence } A = 3.
\]


If $x = -3$, we get:
\[
16 = (A \times 0) + (B \times 0) + C(-2 \times -4);\quad \text{whence } C = 2.
\]
...
\emph{Case II.} If some of the factors of the denominator
...
dealing here, the coefficients of the same powers of $x$
...
\end{align*}
we have $1=A+C$; $0=B+D$ (the coefficient of $x^2$
...
\emph{Case III.} When, among the factors of the denominator
...
\begin{align*}
\frac{dy}{dx}
  &= -\frac{(6x^2+7x-3) \times 4 + (5 - 4x)(12x + 7)}{(6x^2 + 7x - 3)^2}\\
...
partial fractions a precious auxiliary.

\section{Exercises XI}
...
Consider the function %(see <a href="3.html#function">here</a>)
 $y = 3x$; it can be
...
Exercises I. %(<a href="4.html#ex1">here</a>), Nos. 5, 6, 7; Examples (<a href="9.html#ExNo1">here</a>),
%Nos. 1, 2, 4; and Exercises VI. (<a href="9.html#ex6">here</a>), Nos. 1, 2, 3
%and 4.
...
%<a href="14.html">Next &rarr;</a>
%<a href="/">Main Page &uarr;</a>
